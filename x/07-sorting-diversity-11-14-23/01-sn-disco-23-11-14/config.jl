...
sorting_net_parents::Int = 20
sorting_net_elite::Int = 20
test_case_pop::Int = 2000
test_case_parents::Int = 20
...
eco_creator = BasicEcosystemCreator(
    ...
    species_creators = [
        PhylogeneticSpeciesCreator(
            ...
            # TODO add max sort fitness to sorting outcome evaluator
            # evaluator = SortingOutcomeEvaluator(max_sort_fitness=n_test_cases_per_parasite),
            evaluator = OutcomeNSGAIIEvaluator(
                maximize = true, perform_disco = true, max_clusters = 5,
            ),
            replacer = TruncationReplacer(n_truncate = sorting_net_pop),
            selector = TournamentSelector(
                n_parents = sorting_net_parents, 
                tournament_size = 3,
            ),
