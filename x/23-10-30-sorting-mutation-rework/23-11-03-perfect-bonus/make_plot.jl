...
nc = NameConfig()
iderrs = [rolling(agg(load(InteractionDistanceErrors(1:6), nc, pi)), window_size=20) for pi in p]
...
fitness_plots_network = plot(sfs[1][2], title=sfs[1][2].xname)
...
fitness_plots_tc = plot(sfs[1][1], title=sfs[1][1].xname)
